...
public class CannibalsMissionariesController : MonoBehaviour
{
    ...
    void MoveBoat()
    {
        ...
        if (!IsValidMove())
        {
            // Show error message briefly, then show lose screen
            if (ErrorText != null)
            {
                ErrorText.text = "Invalid move! Missionaries would be outnumbered!";
            }

            StartCoroutine(ShowLoseAfterInvalidMove());
            return;
        }
    ...
    IEnumerator ShowLoseAfterInvalidMove()
    {
        // Wait a brief moment to show the error message
